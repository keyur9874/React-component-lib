  ...
  (
    ...
    const uploadFile = useCallback(
      (file: UploadFile) => {
        ...
        if (customRequest) {
          const requestData =
            typeof data === "function" ? data(file.originFileObj!) : data;
          ...
          if (file.type.startsWith("image/")) {
            ...
            reader.onload = (e) => {
              ...
              const updatedList = currentFileList.map((f) =>
                f.uid === newUploadFile.uid
                  ? { ...f, thumbUrl: newUploadFile.thumbUrl }
                  : f
              );
    ...
    const classes = [baseClass, sizeClass, typeClass, className]
      .filter(Boolean)
      .join(" ");
          ...
          {currentFileList.map((file) => {
            ...
            return (
              <div key={file.uid} className={itemClasses}>
                <div className="ui-upload-file-icon">{getFileIcon(file)}</div>

                <div className="ui-upload-file-info">
                  <div className="ui-upload-file-name">{file.name}</div>
                  <div className="ui-upload-file-size">
                ...
                <div className="ui-upload-file-status">
                  {file.status === "uploading" && (
                    <Loader2
                      size={16}
                      className="ui-upload-status-icon ui-upload-status-icon--uploading"
                    />
                  )}
                  {file.status === "done" && (
                  ...
                  {file.status === "error" && (
                    <XCircle
                      size={16}
                      className="ui-upload-status-icon ui-upload-status-icon--error"
                    />
                  )}
